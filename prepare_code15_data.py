...
def run(args):
    ...
    df = pd.read_csv(str(Path(args.demographics_file)))
    ...
    df = pd.read_csv(str(Path(args.labels_file)))
    for idx, row in df.iterrows():
        exam_id = row['exam_id']
        assert(is_integer(exam_id))
        exam_id = int(exam_id)
        ...
        with h5py.File(signal_file, 'r') as f:
            ...
            for i in range(num_exam_ids):
                ...
                wfdb.wrsamp(record, fs=sampling_frequency, units=[units]*num_leads, sig_name=lead_names,
                            ...
                            write_dir=str(Path(args.output_path)), comments=comments)

                if args.signal_format in ('mat', '.mat'):
                    convert_dat_to_mat(record, write_dir=str(Path(args.output_path)))

                # Recompute the checksums as needed.
                checksums = np.sum(digital_signals, axis=0, dtype=np.int16)
                fix_checksums(os.path.join(str(Path(args.output_path)), record), checksums)
